...
/* PENTING: DrawOTag menggambar dari index TINGGI ke RENDAH.
   Jadi index tinggi = digambar duluan = paling BELAKANG.
   Index rendah = digambar terakhir = paling DEPAN. */
enum { L_HUD = 0, L_FX = 1, L_PLAYER = 2, L_BULLET = 3, L_ENEMY = 4,
       L_PLANET = 5, L_STAR = 6, L_BG = 7 };
...
/* Tinggi HUD dan posisi teks (turun dari tepi atas layar) */
#define HUD_H      30
#define TEXT_Y     12
...
static void drawBackground(int frame) {
    ...
    /* Nebula dikerjakan di layer BG juga; ditambahkan SETELAH gradasi
       sehingga tergambar di atasnya (dalam satu layer, yang ditambah
       belakangan tergambar lebih awal - lihat catatan di bawah). */
    int pulse = 20 + sinI(frame / 6) / 12;
    diamond(L_STAR, 80, 70, 90, 60, pulse + 30, 8, pulse + 50, 4, 4, 30);
    diamond(L_STAR, 250, 150, 80, 55, 8, pulse + 40, pulse + 40, 4, 4, 30);
...
/* ---------- HUD (turun agar teks tidak terpotong) ---------- */

static void drawHUD(int lives, int level) {
    rectGradV(L_HUD, 0, 0, SCREEN_W, HUD_H, 10, 20, 50, 2, 2, 14);
    rect(L_HUD, 0, HUD_H, SCREEN_W, 1, 60, 140, 255);

    for (int i = 0; i < START_LIVES; i++) {
        int lx = SCREEN_W - 26 - i * 18;
        if (i < lives) {
            tri(L_HUD, lx + 5, 9,  255, 255, 255,
                       lx,     21, 60, 120, 230,
                       lx + 10, 21, 60, 120, 230);
        } else {
            rect(L_HUD, lx + 3, 16, 4, 4, 40, 40, 60);
        }
    }

    for (int i = 0; i < 10; i++) {
        if (i < level)
            rect(L_HUD, 8 + i * 9, 25, 7, 3, 0, 220, 130);
        else
            rect(L_HUD, 8 + i * 9, 25, 7, 3, 30, 40, 60);
    }
}
...
static void drawMenu(int frame) {
    /* Judul: blok besar gradasi + aksen */
    rectGradV(L_HUD, 40, 50, 240, 44, 255, 150, 40, 200, 40, 20);
    rect(L_HUD, 40, 50, 240, 2, 255, 255, 200);
    rect(L_HUD, 40, 92, 240, 2, 120, 20, 10);

    /* Pesawat besar di tengah, naik-turun pelan */
    int bob = sinI(frame / 2) / 20;
    drawPlayer(SCREEN_W / 2 - 8, 130 + bob, frame);

    /* Garis hiasan */
    rect(L_HUD, 60, 108, 200, 1, 60, 140, 255);
}
...
int main(void) {
    ...
    while (1) {
        ...
        if (state == STATE_MENU) {
            ...
            /* GAME OVER: START kembali ke menu */
            if (!(btn & PAD_START)) {
                state = STATE_MENU;
            }
        }
